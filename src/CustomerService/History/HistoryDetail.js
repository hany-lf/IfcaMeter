...
class HistoryDetail extends Component {
  ...
  getSrfDetail = () => {
    ...
    fetch(urlApi + "c_srf/getHistory/IFCAPB/" + report_no , {
      method: "GET",
    })
      .then((response) => response.json())
      .then((res) => {
          console.log('Data Update',res.DataHistory);
        if (res.Error === false) {
          let resData = res.Data;
          let resImages = res.DataImages;
          let resHistory = res.DataHistory;
          ...
          this.setState({ picturehistory: resHistory});
          ...
          this.setState({ problem_cause: resData[0].problem_cause});
          this.setState({ action_taken: resData[0].action_taken});
          this.setState({ remarks: resData[0].remarks});
          console.log('status',this.state.picturehistory.length);
        } else {
            console.log('ERROR');
        }
      })
...
{/*             
            ...
            >
               <View style={{flexDirection: 'row', paddingTop: 10}}>
                    ...
                    <View style={{alignContent: 'space-between', paddingRight: 23, }}>
                        <Text style={{ fontFamily: 'Palatino-Bold'}}>{this.state.status == 'X' ?  'Cancel' : 
                        this.state.status == 'P' ?  'Proses' :
                        this.state.status == 'M' ?  'Modify' :
                        this.state.status == 'F' ?  'Confirm' : 
                        this.state.status == 'Y' ?  'Approved' :
                        this.state.status == 'Z' ?  'Confirm' :  
                        this.state.status == 'A' ?  'Approve' : '' }</Text>
                    </View>
                </View>

                <View style={this.state.status == 'X' || this.state.status == '' ? {display: 'none'} : {paddingTop: 10,paddingLeft: 20,flexDirection: 'row'}}>
                ...
                <View style={this.state.status == 'X' || this.state.status == '' ? {display: 'none'} :{paddingTop: 10, paddingRight: 20,paddingLeft: 20, paddingBottom: 10}}>
                    <View style={{   paddingTop: 17}}>
                ...
                {/* <View style={this.state.status != 'F'  ? {display: 'none'} :{paddingTop: 10}}> */}
                    <View style={this.state.picturehistory.length > 0 ? {  paddingTop: 17, alignContent: 'center', alignItems: 'center'} : {display: 'none'}}>
                        <Text style={{fontFamily: 'Palatino-Bold', textDecorationLine: 'underline', fontSize: 15}} >Solved Picture</Text>
                    </View>
                {this.state.picturehistory.map(pict => (
                    <View style={styles.containImageTop}>
                    <Image style={{width: 200, height: 130, borderColor: Colors.hijautua, borderWidth: 3}} source={{uri: pict.file_url}} />
                    </View>
                ))}
