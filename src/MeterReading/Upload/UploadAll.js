...
import { background, fonts, colors } from "@Asset/styles/base";
...
class UploadAll extends Component {
  ...
  getData = async () => {
    ...
    if (dataAsyncSave) {
      ...
      const dataSaves = dataMeters.filter(
        (item) =>
          item.entity.trim() == dataTowers[0].entity_cd &&
          item.project.trim() == dataTowers[0].project_no &&
          item.meterType == this.state.selType
      );

      const dataAl = dataAlls.filter(
        (item) =>
          item.meter_type == this.state.selType &&
          item.project_no.trim() == dataTowers[0].project_no &&
          item.entity_cd.trim() == dataTowers[0].entity_cd
      );
      const dataCounts = {
  ...
  uploadData = (data) => {
    ...
    const formData = {
      ...
      curr_read_high: null,
      audit_user: this.state.name.replace(/"/g,''),
    };
  ...
  uploadPhoto = (data) => {
    ...
    dataPict.map((images, index) => {
      ...
      RNFetchBlob.fetch(
        "POST",
        urlApi + "c_meter_utility/saveAttachment/IFCAPB",
        {
  ...
  delete(key) {
    const dataMeters = this.state.dataMeter.filter(
      (item) => item.meterId !== key.meterId
    );
    const dataSaves = this.state.dataSave.filter(
      (item) => item.meterId !== key.meterId
    );
    const dataAlls = this.state.dataAll.filter(
      (item) => item.meter_type == this.state.selType
    );
    const dataCounts = {
  ...
  handleChangePicker = (ref) => {
    ...
    const dataAll = this.state.dataAll.filter(
      (item) =>
        item.meter_type == this.state.selType &&
        item.entity_cd.trim() == sel.entity_cd &&
        item.project_no.trim() == sel.project_no
    );
    const dataMeters = dataMeter.filter(
      (item) =>
        item.entity.trim() == sel.entity_cd &&
        item.project.trim() == sel.project_no &&
        item.meterType == this.state.selType
    );
    const dataSaves = dataMeter.filter(
      (item) =>
        item.entity.trim() == sel.entity_cd &&
        item.project.trim() == sel.project_no &&
        item.meterType == this.state.selType
  ...
  handleChangeTab = (tab) => {
    ...
    const dataAlls = ts.dataAll.filter(
      (item) =>
        item.meter_type == tab &&
        item.entity_cd.trim() == sel.entity_cd &&
        item.project_no.trim() == sel.project_no
    );
    const dataSaves = this.state.dataMeter.filter(
      (item) =>
        item.entity.trim() == sel.entity_cd &&
        item.project.trim() == sel.project_no &&
        item.meterType == tab
  ...
  handleAlert = (fnName, data) => {
    ...
    Alert.alert("Alert", "Are you sure to " + fun + " ? ", [
        ...
        <View style={Styles.view}>
          <View style={Styles.listViewContent}>
            <TouchableOpacity
              ...
              onPress={() => this.showPhotos(item)}
            >
              {item.dataPict.length !== 0 ? (
                <Image
                  style={Styles.viewPhotoIcon}
                  source={{ uri: item.dataPict[0].uri }}
                />
              ) : (
                console.log("ok")
              )}
            ...
            <View style={Styles.viewContent}>
              ...
              >
                Meter ID : {item.meterId} - {item.unitNo}
              </Text>
              <View style={Styles.textWrap}>
                <Text style={Styles.text}>
                  Last Read : {item.lastRead} {satuan[item.meterType]}
                </Text>
              </View>
              <View style={Styles.textWrap}>
                <Text style={Styles.text}>
                  Read : {item.meteran} {satuan[item.meterType]}
                </Text>
              </View>
              ...
              {item.cpName.map((val, key) => (
                <View key={key} style={Styles.textWrap}>
                  <Text style={Styles.text}> - {val.debtor_name}</Text>
                </View>
              ))}
              <View style={Styles.textWrap}>
                <Text style={[Styles.text, { color: "#adadad" }]}>
                  <Icons size={15} style={{ color: "#adadad" }} name="md-time" />{" "}
                  {moment(item.readingDate).format("DD-MMMM-YYYY")}
                </Text>
              </View>
              ...
              <View style={Styles.listBottom}>
                <TouchableOpacity
                  ...
                  onPress={() => this.handleAlert("delete", item)}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => this.handleAlert("uploadData", item)}
                >
                  <Text style={{ fontFamily: "Montserrat-Regular" }}>
                    Upload
                  </Text>
  ...
  render() {
    ...
    const tabColor = {
      ...
      F: { backgroundColor: Colors.hijaumuda },
    };
      ...
      <View style={styles.container}>
                <Header style={Style2.navigation}>
                    ...
                    <View style={Style2.actionBarMiddle}>
                        <Text style={Style2.actionBarText}>{"Upload".toUpperCase()}</Text>
                    </View>
                    <View style={Style2.actionBarRight}></View>
                </Header>
        <View style={styles.top}>
          <RoundedView
            renderContent={this.renderPicker("Tower")}
            width="85%"
            height="8%"
          />
        </View>
            ...
            <View style={Styles.cardContainer}>
              <View style={Styles.cardHead}>
                <TouchableOpacity
                  style={[
                    Styles.tab,
                    tabColor[this.state.selType == "E" ? "T" : "F"],
                    tabRadius["left"],
                  ]}
                  onPress={() => this.handleChangeTab("E")}
                >
                  <Text style={{ fontFamily: "Montserrat-Regular" }}>
                    Electric
                  </Text>
                ...
                <TouchableOpacity
                  style={[
                    Styles.tab,
                    tabColor[this.state.selType == "W" ? "T" : "F"],
                  ]}
                  onPress={() => this.handleChangeTab("W")}
                >
                  <Text style={{ fontFamily: "Montserrat-Regular" }}>
                    Water
                  </Text>
                ...
                <TouchableOpacity
                  style={[
                    Styles.tab,
                    tabColor[this.state.selType == "G" ? "T" : "F"],
                    tabRadius["right"],
                  ]}
                  onPress={() => this.handleChangeTab("G")}
                >
              ...
              <View style={Styles.cardContent}>
                <View style={Styles.cardContext}>
                  <Text style={Styles.textTitle}>
                    {this.state.selTab} Summary
                  </Text>
                  <Text style={Styles.text}>
                    Project : {ts.selTower.project_descs}
                  </Text>
                  <Text style={Styles.text}>
                    Total Reading : {ts.dataCount.total}
                  </Text>
                  <Text style={Styles.text}>
                    Reading : {ts.dataCount.reading}
                  </Text>
                  <Text style={Styles.text}>
                    Unreading: {ts.dataCount.unreading}
                  </Text>
                </View>
              </View>
              <View style={Styles.cardFooter}>
                <View style={Styles.cardBottom}>
                  ...
                  <TouchableOpacity
                    style={Styles.btnWhite}
                    onPress={() => this.handleAlert("uploadAll")}
                  >
                    <Text style={{ fontFamily: "Montserrat-Regular" }}>
                      Upload All
                    </Text>
          ...
          <View style={[{ flex: 1 }, background.primary]}>
            <View style={Styles.headerModal}>
              <Text
                style={[
                  Style.textGreyLight,
                  { fontSize: fonts.md, textAlign: "center" },
                ]}
              >
                Meter Type : {type[this.state.selType]}
              </Text>
            </View>
            ...
            <TouchableOpacity
              ...
              onPress={() => this.setState({ selModal: false })}
            >
              <Icons size={30} name="md-close" />
            </TouchableOpacity>
          </View>
        </Modal>
        <Modal
        ...
        >
          <View style={Styles.photoViewer}>
            <View style={Styles.photoViewerContent}>
              <TouchableOpacity
                style={styles.iconBack}
                onPress={() => this.setState({ viewI: false })}
              >
                <Icons size={25} name="md-close" />
              </TouchableOpacity>
              <View style={Styles.photoWrap}>
                <Text style={{ textAlign: "center", fontWeight: "bold" }}>
                  {this.state.selPhotos.id}
                </Text>
